...
3. 沿已验证的 `run.toolchain.json` 垂直切片继续收敛 Kernel 入口：Bun/PowerShell 已组合 Archive Tool、类型化声明、环境脚本与 Provider CAS，形成可离线验收、可注入下载进度、失败保持 unavailable 的内部原生编排服务；MSVC 的来源、缓存、安装、恢复、发布与环境适配也已形成原生领域服务。下一批先把 MSVC 接入同一总编排，再迁移 Rust 生产安装与环境适配；四个默认领域都闭环后，才一次切换 `.dev.setup` handler 并删除对应 PowerShell 日常入口，不注册半成品命令或临时桥接协议。冷 Bootstrap 的 Rust/MSVC 准备逻辑继续保留在系统原生 Shell。Action 可迁移到 Entry 环境提供的 `run.ts`。删除通用 `run.ps1` / `run.cmd` 适配器前，再补齐 `run.py` 的运行时决策和剩余迁移清单，避免长期兼容层。
