...
4. 当前执行器支持 `run.exe`、由当前 Entry 开发环境执行的 Action `run.ts`、Kernel 专用 `run.toolchain.json`、`run.ps1` 和受限的 `run.cmd`。Action `run.ts` 严格解析 Entry Profile 与 `.dev.setup` Provider Export，不使用系统 PATH；受管 Bun 在 Guard 完成后、目标进程启动前才做最终解析和完整性复验，避免 Guard 修复或改写环境后沿用旧结果。`run.toolchain.json` 不执行目录脚本，而是固定调用同 Release Set 的 `swawkit-proj-toolchain.exe command-v1 <handler>`，Catalog 同时限制 handler 白名单与 Kernel 所有权。`.dev.status` 已完成首个垂直迁移：原 PowerShell 入口已删除；Bun/PowerShell 的 selection、安装元数据、文件清单、重解析点、完整 SHA-256 与信任分类已经收敛为一套 Archive Tool SSOT，其中 Core `run.ts` 消费 Bun，Toolchain `.dev.status` 消费 Bun 与 PowerShell。MSVC 也已形成独立的 Rust 原生领域闭环：类型化定义、Channel/Manifest 解析、受限 Microsoft 来源、多制品内容寻址缓存、VSIX 安全解包、MSI/CAB 组装、环境映射、精确文件清单、完整 SHA-256、中断恢复与原子发布共用同一读写契约；已就绪或可恢复的安装保持完全离线。Rust 的类型化定义、规范元数据、完整工具链清单、哈希复验与隔离环境映射也已收敛为共享领域 SSOT，`.dev.status` 仅保留薄适配；生产下载与安装事务仍待迁移。Archive Tool 的共享下载缓存、内容身份隔离、ZIP 安全校验、安装发布、干净重试与中断恢复也已成为可复用的 Rust 内部服务；它会先离线识别或恢复已就绪安装，只在确实缺少安装时解析来源，并用同一套元数据/完整哈希 SSOT 复验候选。GitHub Release 的 exact/latest 解析、规范下载坐标、Project/GitHub/unverified 信任优先级和受限网络读取也已收敛进该服务；来源信任不能由普通调用者自行标记，`latest` selection 只会在安装完整验证成功后以实际内容摘要不可变发布，重复 setup 不会暗中升级。开发工具声明及其 Profile input revision 已共享同一套类型化注册表；环境脚本的有序生成、字节稳定发布和 `.dev.setup` 专用 Provider start/complete CAS 也已成为 Rust 内部服务，Core 与 Toolchain 对 Provider State 的读取共用相同严格协议。Bun、PowerShell 与 MSVC 现已接入同一个内部原生总编排：共用 setup 锁和 Provider CAS，先完整解析所有启用声明，再离线优先解析或恢复安装，按固定领域顺序生成环境，发布环境后才完成 ready。Rust 生产安装领域尚未迁移，因此默认 Profile 下的完整 `.dev.setup` 入口继续由既有 PowerShell 主路径负责，尚未注册会丢失领域能力的半成品 Rust handler；只有冷 Bootstrap 在 Toolchain 尚不存在时保留系统原生 Shell 实现。
...
3. 沿已验证的 `run.toolchain.json` 垂直切片继续收敛 Kernel 入口：Bun、PowerShell 与 MSVC 已组合各自领域服务、类型化声明、环境脚本与 Provider CAS，形成可离线验收、可注入下载进度、失败保持 unavailable 的内部原生总编排。下一批迁移 Rust 生产安装与环境适配；四个默认领域都闭环后，才一次切换 `.dev.setup` handler 并删除对应 PowerShell 日常入口，不注册半成品命令或临时桥接协议。冷 Bootstrap 的 Rust/MSVC 准备逻辑继续保留在系统原生 Shell。Action 可迁移到 Entry 环境提供的 `run.ts`。删除通用 `run.ps1` / `run.cmd` 适配器前，再补齐 `run.py` 的运行时决策和剩余迁移清单，避免长期兼容层。
