...
4. 当前执行器支持 `run.exe`、由当前 Entry 开发环境执行的 Action `run.ts`、Kernel 专用 `run.toolchain.json`、`run.ps1` 和受限的 `run.cmd`。Action `run.ts` 严格解析 Entry Profile 与 `.dev.setup` Provider Export，不使用系统 PATH；受管 Bun 在 Guard 完成后、目标进程启动前才做最终解析和完整性复验，避免 Guard 修复或改写环境后沿用旧结果。`run.toolchain.json` 不执行目录脚本，而是固定调用同 Release Set 的 `swawkit-proj-toolchain.exe command-v1 <handler>`，Catalog 同时限制 handler 白名单与 Kernel 所有权。`.dev.status` 已完成首个垂直迁移：原 PowerShell 入口已删除；Bun/PowerShell 的 selection、安装元数据、文件清单、重解析点、完整 SHA-256 与信任分类已经收敛为一套 Archive Tool SSOT，其中 Core `run.ts` 消费 Bun，Toolchain `.dev.status` 消费 Bun 与 PowerShell；MSVC、Rust 的状态检查仍按各自领域由 Rust 承担。Archive Tool 的共享下载缓存、内容身份隔离、ZIP 安全校验、安装发布、干净重试与中断恢复也已成为可复用的 Rust 内部服务；它会先离线识别或恢复已就绪安装，只在确实缺少安装时解析来源，并用同一套元数据/完整哈希 SSOT 复验候选。GitHub Release 的 exact/latest 解析、规范下载坐标、Project/GitHub/unverified 信任优先级和受限网络读取也已收敛进该服务；来源信任不能由普通调用者自行标记，`latest` selection 只会在安装完整验证成功后以实际内容摘要不可变发布，重复 setup 不会暗中升级。当前 `.dev.setup` 的声明编排、环境脚本与 Provider 发布仍由既有 PowerShell 主路径负责，尚未注册半成品 Rust handler；只有冷 Bootstrap 在 Toolchain 尚不存在时保留系统原生 Shell 实现。
...
3. 沿已验证的 `run.toolchain.json` 垂直切片继续收敛 Kernel 入口：Archive Tool 的持久状态、GitHub Release/exact/latest 解析、可信来源、共享下载缓存、安装事务、中断恢复和成功后 selection 发布已经成为 Rust 内部服务。下一批补齐类型化开发工具声明、环境脚本生成与 Provider 发布，并让 Bun/PowerShell 先形成完整的原生 `.dev.setup` 闭环；随后纳入仍受支持的 MSVC、Rust 生产路径，最后一次切换 handler 并删除对应 PowerShell 日常入口，不注册半成品命令或临时桥接协议。冷 Bootstrap 的 Rust/MSVC 准备逻辑继续保留在系统原生 Shell。Action 可迁移到 Entry 环境提供的 `run.ts`。删除通用 `run.ps1` / `run.cmd` 适配器前，再补齐 `run.py` 的运行时决策和剩余迁移清单，避免长期兼容层。
