...
4. 当前 Host 的 run registry 只负责活跃运行、取消和短期内存窗口；持久事实由模块 `_runs/` 中的 Run Journal 承担。CLI 在执行器边界边回显边记录，Web 则由 Host 在 Worker 外层记录，因此 Worker 启动失败和取消也能形成一致终态。Web 的日志视图通过 `/api/v2/command-run-journals` 读取最近 32 次摘要和游标增量事件；命令身份由单值 `command=kernel/.dev.status`（或 `action/proj.build`）同时指定来源与地址，Catalog 一次扫描即可定位模块，读取历史不要求命令当前仍可运行。界面将运行记录索引与单次运行内容分为相邻两列；磁盘日志不依赖当前 Host 生命周期。
