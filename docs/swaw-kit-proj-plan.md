...
3. 沿已验证的 `run.toolchain.json` 垂直切片继续收敛 Kernel 入口：Archive Tool 的持久状态读侧已经成为 Rust SSOT，下一批先迁移共享下载缓存、安装事务与中断恢复，再迁移 GitHub Release/latest 解析和 Bun/PowerShell 差异适配；只有环境脚本、Provider 发布和全部受支持工具闭环后，才一次切换 `.dev.setup` handler 并删除对应 PowerShell 日常入口，不注册半成品命令或临时桥接协议。冷 Bootstrap 的 Rust/MSVC 准备逻辑继续保留在系统原生 Shell。Action 可迁移到 Entry 环境提供的 `run.ts`。删除通用 `run.ps1` / `run.cmd` 适配器前，再补齐 `run.py` 的运行时决策和剩余迁移清单，避免长期兼容层。
